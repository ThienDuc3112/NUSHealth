...
export const getRoutineByPlanId = async (
  planId?: number
): Promise<
  { routine: routine["routine"]; targets: string[]; exercisesCount: number }[]
> => {
  ...
  if (!planId && planId != 0) {
    const routines = await db
      ...
      .leftJoin(
        ...
        eq(routineTable.id, exerciseToRoutineTable.routineId)
      )
      .leftJoin(
        exerciseTable,
    ...
    routines.forEach((r) => {
      ...
      if (r.target != null) {
        ...
        if (!temp[r.routine.id].targets.some((target) => target == r.target))
          temp[r.routine.id].targets.push(r.target);
      }
    });
    ...
    const routines = await db
      ...
      .innerJoin(
        routineTable,
        eq(routineToPlanTable.routineId, routineTable.id)
      )
