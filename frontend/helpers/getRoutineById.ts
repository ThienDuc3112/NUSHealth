...
export const getRoutineById = async (routineId: number): Promise<routine> => {
  ...
  const exerciseDir: Record<
    number,
    exercise & typeof exerciseToRoutineTable.$inferSelect
  > = {};
  routinesExercises.forEach((exercise) => {
    const {
      ...
      exercise_photos,
      targeted_muscles,
      exercise_to_routine: e2r,
    } = exercise;
    if (!exerciseDir[e2r.id]) {
      exerciseDir[e2r.id] = { ...ex, targetedMuscles: [], photos: [], ...e2r };
    }
    if (exercise_photos) {
      exerciseDir[e2r.id].photos.push(exercise_photos.url);
    }
    if (targeted_muscles) {
      exerciseDir[e2r.id].targetedMuscles.push(targeted_muscles.muscle);
    }
  });
  return {
    routine: routine[0],
